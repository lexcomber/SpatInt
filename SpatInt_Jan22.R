...
tz_m <- st_difference(tz_sf, st_transform(mask_sf, crs(tz_sf)))
...
ol <- st_intersection(st_transform(streets, crs(sz_sf)), sz_sf)
...
# intersection
ol <- st_intersection(st_transform(buildings, crs(tz_sf)), tz_sf)
...
ol <- st_intersection(st_transform(props_sf, crs(tz_sf)), tz_sf)
ol <- st_intersection(ol, sz_sf)
ol$count <- 1
df <- data.frame(ol[, c("TID", "SID", "count")])[, 1:3]
df <- as.data.frame.matrix(xtabs(count~TID+SID, df))
